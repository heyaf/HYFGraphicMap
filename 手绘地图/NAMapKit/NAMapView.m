...
- (void)centerOnPoint:(CGPoint)point animated:(BOOL)animate
{
    ...
    if (y +self.frame.size.height>self.originalSize.height*self.zoomScale) {
        y=self.originalSize.height*self.zoomScale-self.frame.size.height;
    }
    if (x +self.frame.size.width>self.originalSize.width*self.zoomScale) {
        x=self.originalSize.width*self.zoomScale-self.frame.size.width;
    }
...
//    }
//    ASLog(@"%f,%f计算后的%f,%f----frame(%f,%f)----zoomscal%f",point.x,point.y,x,y,self.frame.size.width,self.frame.size.height,self.zoomScale);
